...
public class TSAccountManagerImpl: TSAccountManager, DatabaseChangeDelegate {
...
extension TSAccountManagerImpl: LocalIdentifiersSetter {
    ...
    public func setIsTransferInProgress(_ isTransferInProgress: Bool, tx: DBWriteTransaction) -> Bool {
        return mutateWithLock(tx: tx) {
            let oldValue = kvStore.getBool(Keys.isTransferInProgress, defaultValue: false, transaction: tx)
            guard oldValue != isTransferInProgress else {
                return false
            }
            if isTransferInProgress {
                Self.regStateLogger.info("Transfer in progress!")
            } else {
                Self.regStateLogger.info("Transfer no longer in progress")
            }
    ...
    public func setWasTransferred(_ wasTransferred: Bool, tx: DBWriteTransaction) -> Bool {
        let oldValue = kvStore.getBool(Keys.wasTransferred, defaultValue: false, transaction: tx)
